...
export default function LoginForm({
  ...
  onEmailChange, onPasswordChange, onTogglePass, onSubmit, onGoogle,
}: LoginFormProps) {
  return (
    <div className="min-h-screen flex overflow-hidden">
      ...
      <div className="hidden lg:flex flex-1 bg-[#0ea5e9] p-10 flex-col justify-between relative overflow-hidden">
        ...
        <div className="z-10">
          <h2 className="font-serif text-6xl font-bold text-white leading-[1.1]">
            ...
            <em className="text-[#6ee7b7] not-italic">style</em>
          </h2>
      ...
      {/* RIGHT PANEL */}
      <div className="w-full lg:w-[360px] flex-shrink-0 bg-white flex flex-col justify-center px-10 py-12 overflow-y-auto">
        <div className="h-1.5 rounded-full bg-gradient-to-r from-[#0ea5e9] via-[#06b6d4] to-[#34d399] mb-8" />
        <h1 className="font-serif text-3xl font-bold text-[#0a1628]">Welcome back</h1>
        <p className="text-[#7a8ea0] text-xs mt-1 mb-8">Sign in to continue your journey</p>

        {error && (
          <div className="bg-red-50 border border-red-200 text-red-600 p-3 rounded-lg text-xs mb-5">
            {error}
          </div>
        )}
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="text-right">
            <a href="/forgot-password" className="text-[11px] text-[#0ea5e9] font-semibold hover:underline">
              Forgot password?
            </a>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-[#0ea5e9] to-[#06b6d4] text-white py-3 rounded-lg text-sm font-semibold disabled:opacity-50 hover:opacity-90 transition-opacity"
          >
